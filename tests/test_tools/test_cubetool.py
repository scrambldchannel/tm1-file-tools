...
def test_get_all_cubes(data_folder):

    ct = TM1CubeFileTool(path=data_folder)

    all_cubes = ct.get_all_cubes()

    assert all_cubes
    assert len(all_cubes)

    assert any(c.stem == "}ElementAttributes_koala" for c in ct.get_all_cubes())

    assert any(c.stem == "cat" for c in ct.get_all_cubes())
