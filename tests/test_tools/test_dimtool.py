...
def test_get_all(data_folder):
    ...
    all_dimensions = dt.get_all()

    assert all_dimensions
    assert len(all_dimensions)

    assert any(d.stem == "}ElementAttributes_koala" for d in dt.get_all())

    assert any(d.stem == "koala" for d in dt.get_all())
